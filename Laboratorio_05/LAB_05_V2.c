...
void main(void){
   ...
   // Detecta si se reseteo por software, si el numero ingresado no es válido
    ...
    while(1){
        ...
        unsigned char Antiguo_boton = 0;  // Variable pra observar el falnco de subida en el sensor de barrera pin 4
        ...
        while(1){
            ...
            unsigned char Actual_RC = RC1; // Lee el estado actual del pin 4
            
            // Deteccion sensor de barrera
            
           
            if( Actual_RC != 0 && Antiguo_boton == 0 && contadas < objetivo){ //Señal del sensor
            ...
            Antiguo_boton = Actual_RC;   // El estado antiguo sera el que tiene ahora     
...
void __interrupt() ISR(void){
    ...
    if(TMR0IF == 1){ 
        ...
        // Perido de 1 segundo, frecuencia de 1 Hz
            // Control automático del potenciómetro
        if(potenciometro_habilitado == 0){
            contador_motor++;
            if(contador_motor >= 10){  // Después de 5 segundos (10 interrupciones)
        ...
        if(LATA1 == 1){
            ...
            TransmitirVar("Valor ADC: ");
            TransmitirVar(numAscii);
        }
                 
            // Control del motor por ADC
            ControlMotorPorADC();
...
void ControlMotorPorADC(void){
    if(potenciometro_habilitado == 0){
        return;  // Salir sin hacer nada si está deshabilitado
    }
    
    unsigned char valADC = ConversionADC(0);
    
    // Si ADC > 127 (2.5V), prender motor
    if(valADC > 127){
        CCPR1L = 240;  // Motor al 100%
        LATC2 = 1;     // Pin adicional si se necesita
    }
    else {
        CCPR1L = 0;    // Motor apagado
        LATC2 = 0;
    }
}